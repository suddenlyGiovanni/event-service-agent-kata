 ...
 *
 * This is required before saving timers in the Live (SQLite) adapter tests, as timers have a foreign key constraint to
 * the service_calls table.
 *
 * **Idempotency**: Uses `INSERT OR IGNORE` to skip insertion if the record already exists. This ensures tests can
 * safely call this helper multiple times with the same keys without causing UNIQUE constraint violations.
 *
