...
/**
 * Internal service for sharing event capture Ref between mock and harness.
 * Not exported - purely internal wiring.
 */
class TestEventCapture extends Context.Tag('test/TestEventCapture')<
	TestEventCapture,
 ...
 *
 * Creates a mock EventBusPort that captures published events to a shared Ref.
 * Provides:
 * - TestEventCapture: Ref for reading captured events in assertions
 * - Platform.EventBusPort: Mock implementation for Timer module
 *
 * Used in `dependencies` for service construction only.
 */
const EventBusTestLayer: Layer.Layer<TestEventCapture | Ports.Platform.EventBusPort, never, never> = Layer.unwrapEffect(
	Effect.gen(function* () {
		const eventsRef = yield* Ref.make(Chunk.empty<MessageEnvelope.Type>())

		const EventBusPortLayer: Layer.Layer<Ports.Platform.EventBusPort, never, never> = Layer.mock(
			Ports.Platform.EventBusPort,
			{
				publish: (events) => Ref.update(eventsRef, (existing) => Chunk.appendAll(existing, Chunk.fromIterable(events))),
				// TODO: implement when needed
				subscribe: () => Effect.never,
			},
		)

		const EventCaptureLayer: Layer.Layer<TestEventCapture, never, never> = Layer.succeed(TestEventCapture, eventsRef)
 ...
 * ```
 */
export class TestHarness extends Effect.Service<TestHarness>()('test/TestHarness', {
	dependencies: [
		Layer.merge(
	...
	effect: Effect.gen(function* () {
		...
		const eventsRef = yield* TestEventCapture
