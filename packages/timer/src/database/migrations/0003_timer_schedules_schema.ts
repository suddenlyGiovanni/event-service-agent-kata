...
const migration: Effect.Effect<void, Sql.SqlError.SqlError, Sql.SqlClient.SqlClient> = Effect.gen(function* () {
	...
	yield* sql`
      ...
      SELECT tenant_id,
             ...
             CASE
                 WHEN COALESCE(state, 'Scheduled') = 'Reached' AND reached_at IS NOT NULL
                     THEN reached_at
                 ELSE NULL
                 END                                             AS reached_at,
             CASE
                 WHEN COALESCE(state, 'Scheduled') = 'Reached' AND reached_at IS NULL
                     THEN 'Scheduled'
