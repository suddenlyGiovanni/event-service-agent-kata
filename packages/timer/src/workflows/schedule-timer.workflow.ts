...
export const scheduleTimerWorkflow: (
	...
	function* (input) {
		yield* Effect.annotateCurrentSpan({
			correlationId: Option.fromNullable(input.correlationId),
			dueAt: input.command.dueAt,
			serviceCallId: input.command.serviceCallId,
		...
		const scheduledTimer = TimerEntry.make(input.command, now, input.correlationId)
