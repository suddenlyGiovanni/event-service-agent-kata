 ...
 *
 * @remarks
 * Exported for adapter implementations and testing. Domain logic should prefer using {@link TimerEntry} namespace
 * helpers (isScheduled, markReached) for type-safe operations.
 */
 ...
 * @remarks
 * Exported for adapter implementations and testing. Domain logic should prefer using {@link TimerEntry} namespace
 * helpers (isReached) for type-safe operations.
 */
 ...
 *
 * Effect Schema automatically discriminates by _tag field, enabling type-safe pattern matching and JSON
 ...
 * import { makeUUID7 } from '@event-service-agent/platform/uuid7'
 *
 * // Timer can be in different states
 * const timer: TimerEntry.Type = new TimerEntry.ScheduledTimer({
 * 	tenantId: TenantId.make(makeUUID7()),
 * 	serviceCallId: ServiceCallId.make(makeUUID7()),
...
export const TimerEntry = {
	 ...
	 *
	 * Note: This is a pure function. Persistence layer handles idempotency (preserves original reachedAt if already
	 * Reached).
	 */
