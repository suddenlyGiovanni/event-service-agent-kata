 ...
 *
 * **Architecture Note**: Contracts cannot import from timer/orchestration/execution
 * to avoid circular dependencies. Instead, we define the Encoded shape of each message.
 * This matches the DTO structure after Schema.encode().
 *
 * Currently includes:
 * - Timer: DueTimeReached
 *
 * TODO (PL-14): Add remaining messages:
...
export const DomainMessage = Schema.Union(
	// Timer Events
	// DueTimeReached.Encoded shape (matches timer/src/domain/events.domain.ts)
	Schema.Struct({
		_tag: Schema.Literal('DueTimeReached'),
		reachedAt: Schema.optional(Schema.String), // Iso8601DateTime (branded) → string
		serviceCallId: Schema.String, // ServiceCallId (branded) → string
		tenantId: Schema.String, // TenantId (branded) → string
	}),
)
