 ...
 *
 * **Domain Model — ServiceCall as Aggregate Root**: ServiceCall is the single aggregate in the MVP system. Every domain
 * event and command that manipulates ServiceCall state carries a ServiceCallId to identify which aggregate instance is
 * being operated on.
 *
 * **Why ServiceCallId exists**:
 *
 * - **Message Ordering**: Used as partition key (with TenantId) to guarantee event ordering per aggregate instance in
 *   message broker
 ...
 *
 * **Per-Aggregate Ordering (see ADR-0002)**: Messages with same (TenantId, ServiceCallId) are processed in order:
 *
 ...
 *
 * Broker ensures ordering by routing messages with same ServiceCallId to same partition. Out-of-order processing would
 * violate state machine invariants.
 *
 ...
 *
 * Identifies a unique instance of the ServiceCall aggregate. All events and commands for this aggregate instance share
 * the same ServiceCallId.
 */
export class ServiceCallId extends UUID7.pipe(Schema.brand(ServiceCallIdBrand)) {
	 ...
	 *
	 * Creates time-ordered identifier for a new ServiceCall aggregate instance. UUID7's timestamp prefix enables
	 * chronological sorting and correlation.
	 *
	 * **When to use**:
	 *
	 ...
	 *
	 * **Application-Generated IDs (per ADR-0010)**: ServiceCallId is generated by API module when receiving HTTP request,
	 * enabling:
	 *
