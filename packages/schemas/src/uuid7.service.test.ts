...
import { UUID7Service } from './uuid7.service.ts'

describe('UUID7Service', () => {
	describe('UUID7Service.Default', () => {
		it.effect('should generate valid UUID v7', () =>
			Effect.gen(function* () {
				expect(yield* UUID7Service.randomUUIDv7()).toMatch(UUID7Regex)
			}).pipe(Effect.provide(UUID7Service.Default)),
		)
		...
		it('should generate UUIDs with increasing timestamps (time-ordered)', async () => {
			const program = Effect.gen(function* () {
				const { randomUUIDv7 } = yield* UUID7Service
	...
	describe('UUID7Service.Test', () => {
		...
		it.effect(
			...
			() =>
				...
				}).pipe(Effect.provide(UUID7Service.Test(fixedUuid))), // Inject test adapter with fixed UUID)
		)
		...
		it.effect('should return same UUID on multiple calls', () =>
			...
			}).pipe(Effect.provide(UUID7Service.Test(fixedUuid))),
		)
		...
		it.effect('should fail if fixed UUID is invalid', () => {
			...
			}).pipe(Effect.provide(UUID7Service.Test(invalidUuid)))
		})
	})

	describe('UUID7Service.Sequence', () => {
		it.effect('should generate sequential UUIDs with incrementing counter', () =>
			Effect.gen(function* () {
				const [uuid1, uuid2, uuid3] = yield* Effect.all([
					UUID7Service.randomUUIDv7(),
					UUID7Service.randomUUIDv7(),
					UUID7Service.randomUUIDv7(),
				])
		...
		it.effect('should use default prefix "00000000" when not provided', () =>
			Effect.gen(function* () {
				const [uuid1, uuid2] = yield* Effect.all([UUID7Service.randomUUIDv7(), UUID7Service.randomUUIDv7()])
			...
			}).pipe(Effect.provide(UUID7Service.Sequence())),
		)
		...
		it.effect(
			...
			() =>
				...
				}).pipe(Effect.provide(UUID7Service.Sequence('aaaabbbb'))), // Inject test adapter with fixed UUID
		)

		it.effect('should generate multiple sequential UUIDs in a single program', () =>
			Effect.gen(function* () {
				for (let i = 0; i < 5; i++) {
					const uuid = yield* UUID7Service.randomUUIDv7()
					expect(uuid).toBe(`abcdef12-0000-7000-8000-00000000000${i}` as UUID7Schema.Type)
				}
			}).pipe(Effect.provide(UUID7Service.Sequence('abcdef12'))),
		)
	})
})
