...
export class DueTimeReached extends Schema.TaggedClass<DueTimeReached>()('DueTimeReached', {
	 ...
	 *
	 * Expects structured DTO with correct field names, not arbitrary unknown. For JSON strings, use
	 * Schema.parseJson(DueTimeReached) instead.
	 *
 ...
 *
 * Payload uses DomainMessage union (all events + commands). Effect Schema automatically validates against all union
 * members.
 *
 * This enables:
 *
...
// publishDueTimeReached: (event: DueTimeReached.Type) => Effect<void, PublishError, MessageMetadata>
