...
// Encode to DTO
const dto = yield* ServiceCallSubmitted.encode(event);

// Wrap in validated envelope (makeEnvelope generates EnvelopeId)
const envelope = yield* makeEnvelope('ServiceCallSubmitted', dto, {
  tenantId,
...
**Rationale:** Orchestration is the aggregate owner (ServiceCall). It receives ServiceCallId from API (preferred per ADR-0010) or generates via UUID7 if missing. EnvelopeId generated by `makeEnvelope` helper for broker deduplication. See [ADR-0010][] for identity generation strategy and [ADR-0011][] for schema patterns.
